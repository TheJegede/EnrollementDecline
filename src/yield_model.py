...
Target: admit_to_enroll. XGBoost primary + LogisticRegression baseline.
70/15/15 stratified split, SMOTE on training only, Platt calibration, top-3 SHAP per prediction.
"""
...
NUMERIC_FEATURES = [
    ...
    "aid_package_amount",
    "scholarship_offer_flag",
    "days_to_deposit_deadline",
    "peer_admit_count",
]
...
def train(df: pd.DataFrame) -> dict[str, Any]:
    """Train yield prediction models on admitted applicants only."""
    set_seeds()

    df_admits = df[df["admitted"] == 1].reset_index(drop=True)

    X = df_admits[FEATURE_COLS]
    y = df_admits[TARGET].values
    ...
    # SMOTE on training set only — yield ~25% positive, meaningful imbalance
    smote = SMOTE(random_state=RANDOM_SEED)
    X_train_s, y_train_s = smote.fit_resample(X_train, y_train)

    # XGBoost GridSearchCV (5-fold CV on SMOTE-resampled training set)
    gs = GridSearchCV(
        XGBClassifier(
            eval_metric="aucpr",
    ...
    # Platt calibration on original (non-SMOTE) validation set
    ...
    lr_base.fit(X_train_s, y_train_s)
    ...
    metrics = {
        ...
        "smote_train_size": int(X_train_s.shape[0]),
        "admits_count": int(len(df_admits)),
    ...
    return {
        ...
        "y_test": y_test,
        "test_idx": test_idx,
        "admits_index": df_admits.index.tolist(),
    }


def predict(artifact: dict, df: pd.DataFrame) -> pd.DataFrame:
    """Score admitted applicants. Returns DataFrame with yield_probability and top-3 SHAP."""
    ...
    out["yield_probability"] = (prob * 100).round(1)
    out["yield_score_raw"] = prob.round(4)
    for rank in range(3):
...
def save(artifact: dict, path: Path | None = None) -> Path:
    out = path or MODELS_DIR / "yield_prediction.pkl"
...
def load(path: Path | None = None) -> dict:
    return joblib.load(path or MODELS_DIR / "yield_prediction.pkl")
